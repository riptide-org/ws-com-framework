...
impl TryFrom<Message> for WebsocketMessage {
    ...
    fn try_from(s: Message) -> Result<Self, WebsocketMessageError> {
        if s.is_binary() {
            bincode::deserialize(s.as_bytes())
                .map_err(|_| WebsocketMessageError::FailedSerialization)
        } else {
...
impl From<WebsocketMessage> for Message {
    fn from(s: WebsocketMessage) -> Message {
        match bincode::serialize(&s)
            .map_err(|e| WebsocketMessage::Error(format!("Failed to serialize message: {}", e)))
        {
            Ok(f) => Message::binary(f),
            Err(e) => Message::binary(
                bincode::serialize(&WebsocketMessage::Error(e.to_string())).unwrap(),
            ),
        }
    }
}
